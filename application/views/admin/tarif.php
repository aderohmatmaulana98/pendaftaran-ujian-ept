								...
								<?php foreach ($tarif as $t) : ?>
									<tr>
										...
										<td>
											...
											<!-- Edit -->
											<a data-toggle="modal" data-target="#editTarif<?= $t['id']; ?>" class="badge badge-primary" href="">
												...
												<div class="modal-dialog">
													<div class="modal-content">
														<div class="modal-header">
															<h5 class="modal-title" id="editTarifLabel">Edit Biaya</h5>
														...
														<form action="<?= base_url('admin/edit_tarif'); ?>" method="POST">
															<div class="modal-body">
																<div class="form-group">
																	<input type="text" class="form-control" id="tarif" name="tarif" value="<?= $t['tarif'] ?>">
																...
																<div class="form-group">
																	<select name="id_jenis" class="form-control" id="id_jenis" required oninvalid="this.setCustomValidity('data tidak boleh kosong')" oninput="setCustomValidity('')">
																		<option value="" selected>--Pilih Jenis Pendaftar--</option>
																		<?php foreach ($jenis_pendaftar as $jp) : ?>
																			<option value="<?= $jp['id']; ?>" <?php if ($jp['nama_jenis'] == $t['nama_jenis']) {
																													echo 'selected';
																												} ?>><?= $jp['nama_jenis'] ?></option>
																		<?php endforeach ?>
											...
											<!-- Hapus -->
											<a class=" badge badge-danger" onclick="javascript: return confirm('Anda yakin akan menghapus ini? ')" href="<?= base_url('admin/hapus_tarif/' . $t['id']) ?>">
...
<div class="modal fade" id="newTarifModal" tabindex="-1" role="dialog" aria-labelledby="newTarifModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="newTarifModalLabel">Tambah Biaya</h5>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
			<div class="modal-body">
				...
				<form class="user" action="<?php base_url('admin/tarif'); ?>" method="POST">
					<div class="form-group">
						<input type="number" class="form-control" id="tarif" name="tarif" placeholder="Biaya" required oninvalid="this.setCustomValidity('data tidak boleh kosong')" oninput="setCustomValidity('')">
					</div>
					<div class="form-group">
						<select name="id_jenis" class="form-control" id="id_jenis" required oninvalid="this.setCustomValidity('data tidak boleh kosong')" oninput="setCustomValidity('')">
